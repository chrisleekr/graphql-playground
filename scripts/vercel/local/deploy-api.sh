#!/bin/bash
# This script is to build the API service and deploy it to Vercel.
...
source "$SCRIPT_DIR/build-api.sh"

# Deploy the API service to Vercel from the temporary directory
echo "Deploying API in temporary directory: $TEMP_DIR"
vercel deploy --prebuilt --prod --yes --cwd "$TEMP_DIR"
echo "Deployed API in temporary directory: $TEMP_DIR"
