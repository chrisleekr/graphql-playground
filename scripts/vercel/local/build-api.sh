#!/bin/bash
# This script is to build the API service for Vercel and prepare it for deployment.
...
# Set Vercel project ID for the API service
export VERCEL_PROJECT_ID=$VERCEL_PROJECT_ID_API
...
# Pull Vercel project configuration for the API service
vercel pull --environment=production --yes
echo "Pulled projects to temporary directory: $TEMP_DIR"

# Use Vercel CLI to build the API service
vercel build --prod
echo "Built API in temporary directory: $TEMP_DIR"
